% this version replace the  S = fastRadialSymmetryTransform(Ip,
% p.scales(n):2*p.scales(n), p.alpha, p.beta, p.kappa);  with LoG, which
% may yield better result.
function [nuclei properties] = nucleiSegmentationV2_gLoG(I, varargin)
...
%     S = fastRadialSymmetryTransform(Ip, p.scales(n):2*p.scales(n), p.alpha, p.beta, p.kappa);
    ...
    %%______________gLoG begins here_________________________
%     ac=30;    % remove isolated pixels(non-nuclei pixels)
%     [Nmask,cs,rs,A3]=XNucleiSeg_GL_Thresholding(I,ac);      %% thresholding based binarization % show(R)
%     show(Nmask);
    %%% gLoG seeds detection
%     Nmask=ones(size(I,1),size(I,2));
%     p.scales(n):2*p.scales(n)

% remove white background
show(I);
Nmask=I<200;show(Nmask);

    largeSigma=2*p.scales(n);smallSigma=p.scales(n); % Sigma value
    ns=XNucleiCenD_Clustering(I,Nmask,largeSigma,smallSigma);  %% To detect nuclei clumps
    % turn the seed location into binary mask
    seedind=sub2ind([size(I,1),size(I,2)],ns(1,:),ns(2,:));
    mask_seed=zeros(size(I,1),size(I,2));
    mask_seed(seedind)=1; %show(mask_seed); %    
    show(I);
    hold on,plot(ns(2,:),ns(1,:),'y+');hold off;
    S=mask_seed;
    %%______________gLoG ends here_________________________
    
%     LRadialSymmetry = markerControlledWatershedRadialSymmetry(Ip, S, p.h, p.scales(n), p.noBackgroundMarkers);
    LRadialSymmetry = markerControlledWatershed_gLoG(Ip, S, p.h, p.scales(n), p.noBackgroundMarkers); %show(LRadialSymmetry)
    
    [ERadialSymmetry LRadialSymmetry] = bwboundaries(LRadialSymmetry, 'noholes');
    
    PRadialSymmetry = regionProperties(I, LRadialSymmetry);
    
    LRegionalMinima = markerControlledWatershedRegionalMinima(Ip, p.scales(n), p.noBackgroundMarkers);%show(LRegionalMinima)
    
    [ERegionalMinima LRegionalMinima] = bwboundaries(LRegionalMinima, 'noholes');%show(PRegionalMinima)
    ...
    if exist('P','var')
        if ~isempty(P)
            ...
            nuclei{n} = E(accepted); %sum(accepted)
...
function L = markerControlledWatershed_gLoG(I, S, h, n, noBackgroundMarkers)
...
fg=logical(S);
% provisional nuclei map
t = imdilate(fg, strel('disk', 2*n)); %show(t)
...
% modified segmentation function
D = imimposemin(g, fg | bg); %show(D)

% marker-controlled watershed
L = double(watershed(D));%show(L)
...
ranges = {'Solidity', [0.8 1],...
    'MassDisplacement', [0 0.08],...
    'BoundarySaliency', [5 255]
    };
